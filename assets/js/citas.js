...
function seleccionarPaciente(idPaciente, nombreCompleto) {
...
function buscarPacientes() {
    const input = document.getElementById("busqueda_paciente").value.trim();
    const lista = document.getElementById("lista_pacientes");

    if (input.length > 0) {
        const url = "index.php?c=citas&a=AutocompletarPacientes";
            ...
            .then(data => {
                ...
                if (data.length > 0) {
                    data.forEach(paciente => {
                        ...
                        li.className = "list-group-item list-group-item-action paciente-item";
                        li.textContent = `${paciente.cedula} - ${paciente.nombre} ${paciente.apellido}`;

                        li.addEventListener("mousedown", () => {
                            seleccionarPaciente(paciente.id, li.textContent);
                        });
...
// Event listener para buscar pacientes
document.getElementById("busqueda_paciente").addEventListener("keyup", buscarPacientes);

// Agregar evento blur para verificar si el paciente está registrado
document.getElementById("busqueda_paciente").addEventListener("blur", function () {
    let input = this.value;
    let lista = document.getElementById("lista_pacientes");
    ...
    if (input.length > 0 && !pacientes.some(paciente => input === paciente)) {
        alert("Paciente no registrado, verifique los datos");
